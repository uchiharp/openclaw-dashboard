...
app.get('/api/sessions/:sessionId/content', (req, res) => {
  ...
  let agentDirs;
  try { agentDirs = fs.readdirSync(AGENTS_DIR); } catch { return res.json(err(50000, 'cannot read agents dir')); }

  let jsonlPath = null;
  let agentId = null;
  for (const id of agentDirs) {
    const p = path.join(AGENTS_DIR, id, 'sessions', `${sessionId}.jsonl`);
    if (fs.existsSync(p)) { jsonlPath = p; agentId = id; break; }
  }
  if (!jsonlPath) return res.json(err(40400, 'session not found'));
  ...
  try {
    ...
    res.json(ok({ sessionId, agentId, messages: paged, total, offset, limit }));
  } catch (e) {
    res.json(err(50000, 'failed to read session: ' + e.message.slice(0, 200)));
  }
});
